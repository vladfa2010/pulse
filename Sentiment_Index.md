...
### 6.8. Состояние загрузки

Пока данные индекса и персонального статуса не получены, вместо полноэкранного лоадера показывается **скелетон карточки** внутри `SentimentChartCard`.

**Почему:** раньше использовался `min-h-screen` лоадер «Загрузка индекса настроения…», который занимал весь экран, хотя сам блок индекса значительно меньше.

**Реализация:**
- Стеклянная карточка тех же размеров, что и загруженная (`rounded-xl`, neutral glass background/border, `backdrop-filter`).
- Плейсхолдеры заголовка, подзаголовка и блока «Текущий индекс» (`bg-white/5 animate-pulse`).
- Плейсхолдер области графика фиксированной высоты `h-[235px] md:h-[254px]`.
- Плейсхолдеры строки легенды (desktop-only).
- Если `showMetrics === true`, под карточкой показываются 4 плейсхолдера метрик сообщества (`grid grid-cols-2 md:grid-cols-4`).

Это соответствует общему паттерну загрузки каруселей (`bg-[#161616] animate-pulse`) и предотвращает скачки_layout.
...
- [x] Скелетон карточки при загрузке вместо полноэкранного лоадера.
