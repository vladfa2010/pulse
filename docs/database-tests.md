...
### 5. Гвард перевода — `verify:translateGuard`

Файл: `scripts/translation-guard-verify.js`. БД не нужна.

21 проверок валидатора `isGarbageText` (`src/utils/translationGuard.ts`) и веток
парсера `translateBatch` (ТЗ-01/02 v1.2 + правка maxLen по ревью): все 4 формы эха LLM
(сырой запрос, system-промпт, user-message, перевод-ключ) отклоняются и возвращают
оригиналы, валидные ответы во всех трёх форматах (массив / объект с числовыми ключами /
line-by-line) принимаются, длинный саммари (>300) принимается с `maxLen=2000` и
отклоняется с дефолтным 300. Стенд поймал реальный обход гварда: объект-эхо
`{"model": ..., "messages": ...}` проходил object-ветку позиционно — поэтому
ветка принимает только объекты с числовыми ключами.

```bash
npm run verify:translateGuard
```
