...
19 проверок валидатора `isGarbageText` (`src/utils/translationGuard.ts`) и веток
парсера `translateBatch` (ТЗ-01/02 v1.2): все 4 формы эха LLM (сырой запрос,
system-промпт, user-message, перевод-ключ) отклоняются и возвращают оригиналы,
валидные ответы во всех трёх форматах (массив / объект с числовыми ключами /
line-by-line) принимаются. Стенд поймал реальный обход гварда: объект-эхо
