...
> Последнее обновление: 2026-08-22 (TZ-3 / TZ-3.1 / TZ-3.1fix / TZ-3.2 / TZ-3.2fix / TZ-3.3 / TZ-3.4 / TZ-3.8 / TZ-3.9 — публичный график реакции цены, таймзоны бирж, матрица отображения, производительность, правки аудита, компактные поля и тикер в подписи).
...
## График реакции цены в карточке новости (TZ-3 / TZ-3.1 / TZ-3.2 / TZ-3.3 / TZ-3.4 / TZ-3.8 / TZ-3.9)

Файлы:
- Бэкенд: `src/routes/marketPublic.ts`.
- Фронт: `src/components/NewsCard.tsx`, `src/components/NewsReactionChart.tsx`, `src/components/CandleChart.tsx`, `src/pages/NewsFeed.tsx`.

Логика:

1. `NewsCard` запрашивает `/api/market/news-chart?news_id=<id>` только если call-site передал `showChart={true}`. Матрица отображения:
   - `AllNewsCarousel` («Вся лента») — `true`.
   - `NewsFeed.tsx` (страница «Лента») — `true`.
   - `UnreadNewsCarousel` («Это вы ещё не видели») — `false`.
   - `GlobalNewsCarousel` («Общая лента») — `false`.
2. Запрос лениво загружается через `IntersectionObserver` (`rootMargin: 200px`) и кэшируется в React Query (`['newsChart', article.id]`, `staleTime: 5 мин`).
3. Если у новости есть теги с инструментом (`symbol` или `ticker`+`mic`) — рисуется свёрнутый 5-минутный график высотой 180px.
4. `CandleChart` получает опциональный `markTime` (UTC ISO публикации) и рисует янтарную точку на свече, ближайшей к моменту публикации.
5. Подписи оси и выбор торгового дня используют таймзону биржи инструмента (TZ-3.1).
6. Бэкенд возвращает до 3 инструментов в порядке `matched_tags`; при обработке цикл останавливается после 3 собранных инструментов, чтобы не ходить в Finam за лишними (TZ-3.4).
7. Если инструментов несколько — табы переключения по тикеру.
8. Если новость вне сессии — пометка «вне сессии — показан ближайший день YYYY-MM-DD» и точка не рисуется.
9. **Компактные поля графика (TZ-3.8):** `CandleChart` использует `grid.left: 30, grid.right: 6` вместо `56/16`. Поле свечей в карточке 243px растёт с ~171px до ~207px (+21%), подписи цен слева остаются читаемыми и не перекрывают свечи.
10. **Тикер в подписи под графиком (TZ-3.9):** подпись имеет вид `{TICKER} · {Биржа} · время биржи`. Тикер выделен `font-semibold` и берётся как `symbol.split('@')[0]` — консистентно с табами переключения инструментов. При переключении табов подпись обновляется.

### Кэширование ответа `news-chart` (TZ-3.3 / TZ-3.4)

- Бэкенд кэширует полный ответ в памяти процесса (`newsChartCache` в `src/routes/marketPublic.ts`).
- TTL зависит от данных:
...
- При превышении 5000 записей вытесняются 20% самых старых.
- Заголовок `X-Cache: hit|miss`.

### Оптимизации рендера (TZ-3.3 / TZ-3.4)

- `CandleChart` обёрнут в `React.memo`.
- Метки осей и `markPoint` мемоизированы через `useMemo`.
- Двойной поиск ближайшей свечи убран.
- Ресайз слушает `ResizeObserver` на DOM-контейнере, cleanup синхронно отключает observer и уничтожает инстанс ECharts при размонтировании.
