...
### 3.3 Подсказки и тултипы

- **Первый тултип** — поясняет, что `symbol` (`TICKER@MIC`) нужен для маркет-данных, а свободный тикер без `symbol` работает только как текстовая метка.
- **Второй тултип** (TZ-6) — предупреждает, что при сохранении тега в фоне запускается **ретро-сканирование существующих новостей** по keywords тега. Это может занять от секунд до минут; статус сканирования виден в колонке «Scan» на `TagsTab`.

### 3.4 Компонент `InstrumentSearchInput`
