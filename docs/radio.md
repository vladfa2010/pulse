...
(гарантия порядка саммари + защита от race)** ✅.
...
1. **Запуск (ТЗ-53):** ▶ Эфир за один клик — приветствие (время суток, день,
   счётчик) → общее саммари рынка (из кэша `market`, иначе fire-and-forget) →
...
- ТЗ-42/43/44/45/46/47/49/50/53/54, REVIEW_RADIO_TZ42_V31 — исходные ТЗ и вердикт техлида.
