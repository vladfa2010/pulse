...
**ТЗ-46 (kill-switch сервиса)** ✅ · **ТЗ-47 (дефолт autoRead:false)** ✅.
...
| ТЗ-47 дефолт `autoRead: false` + AUTO-бейдж + лимит очереди + кулдаун | ✅ на проде (frontend `08bd1f9`) | 2026-09-23 |
...
Эффективное авточтение = только `blocks.autoRead` (юзер, **дефолт выкл с
ТЗ-47**). Серверного флага
...
- **Стоимость TTS:** дефолтный режим «мысли» = 2 сегмента на карточку;
  авто-поток глобально управляется юзерским `blocks.autoRead` (ТЗ-46; с ТЗ-47
  дефолт выкл — расход Minimax снижен до ручных запусков эфира);
...
с ON AIR при включённом авто-потоке), SettingsPanel (режим на
...
2. **Фон:** SSE-новость → подсветка 4 с + пилик (≥8.5 тройной) → авточтение
   при `blocks.autoRead` (юзер, дефолт выкл ТЗ-47) + кулдаун 30 с после ▶
...
| Блок «Авточтение» (ТЗ-47) | юзерская настройка авто-потока; **дефолт выкл** — SSE-новости в ленте визуально появляются, но в очередь озвучки не встают, пока юзер явно не включит тумблер | **выкл** |
...
Авточтение = только `blocks.autoRead` (юзер, ТЗ-46; **дефолт выкл с ТЗ-47**).
...
2. **Фон:** SSE-новость по тегам → подсветка 4 с + пилик (≥8.5 — тройной) →
   авточтение при юзерском `blocks.autoRead` (ТЗ-46, дефолт выкл ТЗ-47;
...
- ТЗ-42/43/44/45/46/47, REVIEW_RADIO_TZ42_V31 — исходные ТЗ и вердикт техлида.
