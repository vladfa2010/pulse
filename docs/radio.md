...
пропускается, очередь продолжается.
...
- **Стоимость TTS:** дефолтный режим «мысли» = 2 сегмента на карточку;
  авточтение глобально управляется `radio_auto_read_enabled`. Hardcap не делали —
...
| Блок «Авточтение» | юзерский kill-switch поверх серверного флага | вкл |
...
Эффективное авточтение = `radio_auto_read_enabled` (сервер) **И**
`blocks.autoRead` (юзер). Фильтра важности при авточтении нет: все новости по
тегам или никаких (осознанное упрощение v1).
