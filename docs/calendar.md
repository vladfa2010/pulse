...
**Исправлено в М1–М6** (2026-09-05, все 6 скриптов зелёные в обоих режимах, SQLite + PG):
- ✅ Тестовый администратор — фиксированный UUID вместо строки `admin1` (М3–М6).
- ✅ `is_admin` — `TRUE` вместо `1` (строгий boolean PG).
- ✅ `datetime('now')` → `NOW()`, `?` → `$N`, строковые id → UUID, keywords `TEXT[]` — параметрами-массивами.
- ✅ DATE-ассерты — нормализация через `toDateStr()` (PG возвращает Date, SQLite — строку).
- ✅ М5: `await flushCanonicalRewrites()` после загрузки среза — в PG фоновая пересборка канона (DELETE + ~800 INSERT'ов по сети) медленнее SQLite; без ожидания следующий запрос читал канон до её завершения (в SQLite — латентная гонка).
...
- ✅ `schema.sql` — llm-колонки news + сверка runtime-колонок + порядок индексов + `cron_locks` в прод-форме.
- ⚠️ Ограничение bootstrap'а: сплиттер режет schema.sql по `;` до вырезания комментариев — **точка с запятой внутри комментария ломает PG-режим всех скриптов**. В schema.sql комментарии пишем без `;`.

Полный реестр тестов БД и политика DDL: `docs/database-tests.md`.
