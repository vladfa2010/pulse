...
**Правило показа:** баннер виден, если выполнено одно из условий:

- есть замороженные теги (`frozen_tags > 0`);
- активных тегов больше, чем позволяет тариф (`to_remove > 0`).

```
frozen_tags > 0 || to_remove > 0  → баннер виден
frozen_tags = 0 && to_remove = 0  → баннер скрыт
```
...
- `GET /api/user/tags` — возвращает **все** теги (активные + замороженные). Замороженные теги отображаются в портфеле на главной, но не участвуют в ленте новостей.
...
**Автоматическая разморозка при добавлении тега (DEFSUB-17):**

`POST /api/user/tags` перед проверкой лимита вызывает `unfreezeTagsUpToLimit(userId, planId)`. Если после апгрейда/понижения появились свободные слоты, замороженные теги автоматически размораживаются до лимита, и пользователь сразу может добавить новый тег.

**Примеры:**

| Сценарий | active | frozen | limit | to_remove | frozen_tags | Баннер | Заголовок |
|----------|--------|--------|-------|-----------|-------------|--------|-----------|
| Free, 5 тегов | 5 | 0 | 3 | 2 | 0 | **Да** | "У вас слишком много тегов для тарифа" |
| Free, 3 тега | 3 | 0 | 3 | 0 | 0 | **Нет** | — |
| Free, 0 тегов | 0 | 0 | 3 | 0 | 0 | **Нет** | — |
| Base, 15 тегов | 15 | 0 | 10 | 5 | 0 | **Да** | "У вас слишком много тегов для тарифа" |
| Base, 9 тегов | 9 | 0 | 10 | 0 | 0 | **Нет** | — |
| Base, 5 активных + 4 замор. | 5 | 4 | 10 | 0 | 4 | **Да** | "Замороженные теги — выберите активные или восстановите Premium" |
| Free, 3 активных + 4 замор. | 3 | 4 | 3 | 0 | 4 | **Да** | "Замороженные теги — выберите активные или восстановите Premium" |
| Free, 5 активных + 2 замор. | 5 | 2 | 3 | 2 | 2 | **Да** | "Теги заморожены — удалите лишние" |

**Файлы:**
- Frontend: `pulse-frontend/src/components/FreezeTagsBanner.tsx`
- Backend: `pulse-backend/src/services/tagManager.ts` (`getUserTagsFull`), `pulse-backend/src/routes/user.ts` (`GET /api/user/tags`, `POST /api/user/tags`)
...
Возвращает **все** теги пользователя (включая замороженные). Каждый тег содержит флаг `is_frozen`.

Подсчёт новостей за 30 дней (`news_per_month`) ведётся **только по активным тегам** (`is_frozen = FALSE`), поэтому замороженные теги всегда показывают `0` новостей.
...
`news_per_month` — количество уникальных новостей за последние 30 дней, связанных с активным тегом через `news_tag_links`.
