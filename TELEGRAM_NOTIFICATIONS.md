...
> **Актуальная общая архитектура уведомлений** (матрица продукт × канал, entitlement, dispatcher) описана в [`PUSH_NOTIFICATIONS.md`](../PUSH_NOTIFICATIONS.md). Этот документ остаётся справочником по Telegram-каналу.
...
| **Кнопка «Прочитал всё» в дайджесте** | ✅ Реализовано | Inline-кнопка `digest_read_all` на последнем сообщении дайджеста; отмечает все новости прочитанными |
...
|     f. Форматировать в 1-3 Telegram-сообщения (≤3900 символов) │
|     g. Отправить части последовательно (200 мс)         │
|     h. Обновить last_digest_sent только если все части успешно │
+----------------------------------------------------------+
```
...
### Логика разбиения на сообщения

Реализация: `services/digest.ts` → `formatDigestMessages()`.

**Константы:**
- `MAX_MESSAGE_LENGTH = 3900` — бюджет на одно сообщение (Telegram лимит 4096, запас на системные символы).
- `MAX_DIGEST_MESSAGES = 3` — максимальное количество частей.
- `SITE_URL = https://pulse.inside-trade.ru` — ссылка в footer и хвосте.

**Алгоритм:**
1. Каждая статья превращается в блок фиксированного формата:
   ```
   {n}. {emoji} <b>{title}</b>
      📎 <a href="{url}">Читать на сайте</a> · <i>{source}</i>
      🏷 {tag}

   ```
2. Если `header + все блоки + footer` ≤ 3900 символов — отправляется одним сообщением.
3. Иначе сообщения собираются рекурсивно:
   - Первое сообщение содержит `header` и максимум блоков.
   - Промежуточные сообщения продолжают нумерацию, без header/footer.
   - Последнее сообщение вмещает оставшиеся блоки; если не помещаются — в хвосте `…и ещё K статей`.
4. `sendAllDigestMessages()` отправляет части последовательно с паузой 200 мс.
   Если любая часть не дошла, функция возвращает `'error'` и **не обновляет** `last_digest_sent`.

**Почему 3900, а не 4096:**
Telegram считает длину сообщения вместе с невидимыми entity-тегами (`<b>`, `<a ...>`). HTML-ссылки с длинными URL могут занимать больше символов, чем видно в UI. Запас в ~196 символов защищает от граничных случаев.

**Edge cases:**
- Если все статьи помещаются в одно сообщение, но footer не влезает — footer уходит во второе сообщение (редкий случай, обрабатывается рекурсией).
- Если даже 3 сообщений не хватает, последнее частично обрезается хвостом `…и ещё K статей`.
- Заголовок и footer присутствуют только в первом и последнем сообщениях, чтобы не тратить бюджет промежуточных частей.

### Кнопка «✅ Прочитал всё» в дайджесте

После отправки дайджеста под **последним** сообщением добавляется inline-кнопка:
...
**Flow нажатия:**
1. Пользователь нажимает кнопку в любом сообщении дайджеста.
2. Бэкенд получает `callback_query` с `data = 'digest_read_all'`.
3. `getUserIdByChatId()` находит `user_id` по `chat_id`.
4. Вызывается `markAllNewsAsRead(userId)` — та же логика, что и кнопка «Прочитать все» на сайте (`POST /api/news/read-all`).
5. Все непрочитанные новости за 90 дней по активным тегам пользователя записываются в `user_news_reads` (`ON CONFLICT DO NOTHING`).
6. Telegram показывает тост:
   - `✅ Отмечено прочитанными: N. Дальше — только новые новости.`
   - или `✅ Все новости уже прочитаны.` если `marked = 0`.
7. Клавиатура удаляется через `editMessageReplyMarkup` (текст сообщения не меняется).

**Ограничения:**
- Кнопка только на последнем сообщении дайджеста (если дайджест разбит на 2-3 части).
- Работает и в scheduled-дайджесте, и в ручном `/now`, и по кнопке «Получить дайджест» — всё идёт через общий `dispatchToUser()`.
- `callback_data` — `digest_read_all` (15 байт, ≤ 64 байт лимита Telegram).
- Не требует миграций БД: используются `user_news_reads`, `user_channels`, `portfolios`.

### Troubleshooting

| Симптом | Причина | Где смотреть |
|---------|---------|--------------|
| `❌ Ошибка отправки дайджеста.` | Telegram API вернул ошибку (403/400/429) | Логи `sendTelegramMessage`: статус, `error_code`, `description` |
| `📭 Нет новых непрочитанных новостей` | Пул пуст, тихие часы или пользователь/канал не найден | `buildDigest` логи: `candidateCount` |
| Дайджест не приходит, но на сайте есть новости | RSS-окно `/now` = 24ч, scheduled = с `last_digest_sent`; статьи могли быть уже отправлены | Проверить `last_digest_sent` и `fetched_at/published_at` |
| `MESSAGE_TOO_LONG` (400) | Сообщение превысило 4096 символов | Должен быть предотвращён `formatDigestMessages`; если всё же встречается — уменьшить `MAX_MESSAGE_LENGTH` |
...
| **Кнопка «Прочитал всё» (TZ_TG_READ_ALL_BUTTON_v1)** | Inline-кнопка `digest_read_all` на последнем сообщении дайджеста; общая логика `markAllNewsAsRead` вынесена в `services/newsReads.ts` |
...
| `digest_read_all` (inline-кнопка) | ✅ Реализовано | Подключённые | Отмечает все непрочитанные новости прочитанными |
