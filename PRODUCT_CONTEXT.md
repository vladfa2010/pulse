...
- **Правило №8 — единственный источник истины `subscription_expires_at`:**
  ...
  - **Scheduled-даунгрейд на платный тариф (ADM-I):** в момент перехода новый план применяется, но `expires_at` остаётся истёкшим — новый тариф работает на остатке грейса (≤ `GRACE_DAYS` дней) как окно оплаты. Теги сверх лимита нового плана замораживаются сразу в момент даунгрейда (юзер сам выбрал переход с этой даты). Без оплаты по концу грейса юзер уходит на free (lazy fallback через `computeAccessState`). Оплата нового тарифа внутри грейса бесшовна: `activateSubscription` считает период от `NOW()` и размораживает теги. Даунгрейд на free обнуляет `expires_at` через CASE в `applyDowngradeNow` (DEFSUB-16 `5de12bf` + DEFSUB-17 `7fe885a`). Полный неоплаченный период при даунгрейде НЕ выставляется — это коммерчески осознанное решение.
