...
- Интервал фетча: **60 минут** (все тикеры)
...
|---------|----------|------|------------|
| **NSM (RSS)** | **5 мин** | **`nsm`** | **RSS fetch** |
| **NSM (Finnhub)** | **60 мин** | **`nsm`** | **Finnhub fetch (все тикеры)** |
...
|-----|-------|----------------------|
| 403 | Лимит тегов (3 free / **25** premium) | «Tag limit reached (max 3). Upgrade to Premium for more.» |
...
## 9j. Правило тестовых тегов — префикс `dev_`

> **Правило:** Ассистент (Kimi) может создавать тестовые теги ТОЛЬКО с префиксом `dev_`.
> **Цель:** Владелец проекта может отличать тестовые теги ассистента от реальных пользовательских.

### Когда применяется

- Проверка API endpoint'ов (добавление тега)
- Тестирование лимитов (сколько тегов можно добавить)
- Регрессионное тестирование после деплоя
- Проверка работы digest с реальными тегами

### Формат

```
dev_<название>_<timestamp>

Примеры:
  dev_test_tag_1750118400     — тест проверки лимита
  dev_api_check_1750118400    — тест API endpoint
  dev_regression_1750118400   — регрессионный тест
```
...
| # | Правило | Почему |
|---|---------|--------|
| 1 | Префикс `dev_` обязателен | Владелец видит в UI что это тестовый тег |
| 2 | Удалить после использования | Не оставлять мусор в портфеле |
| 3 | Не создавать теги без `dev_` | Риск перепутать с реальными |
| 4 | Не создавать теги в чужих аккаунтах | Только в admin (vladfa@ya.ru) |

### Процедура тестирования

```
1. POST /api/user/tags { tagId: "dev_test_<timestamp>", tagName: "Test", tagType: "company" }
2. Проверить ответ (200 = OK, 403 = лимит, 409 = дубль)
3. DELETE /api/user/tags/dev_test_<timestamp>
4. Проверить что тег удалён (GET /api/user_tags — нет dev_ тега)
```

### История

| Дата | Тег | Зачем | Удалён |
|------|-----|-------|--------|
| 2026-06-16 | `test_tag_25limit` | Проверка лимита 25 | ✅ Да |
...
| Тариф | Теги | Дайджест | Weekly Report | Email | Цена |
|-------|------|----------|---------------|-------|------|
| **Free** | 3 | Нет | Нет | Нет | 0 ₽ |
| **Premium** | **25** | Каждый час | Воскресенье 13:00 | Да | 490 ₽/мес |

### 16.2. Успешный сценарий: расширение Premium 10 → 25 тегов

> **Дата:** 2026-06-16  
> **TZ:** TZ_EXPAND_TAGS_10_TO_25_v2  
> **Коммиты:** `3e03bb1` (backend), `95ac00f` (frontend)

#### Проблема

Лимит 10 тегов на Premium не покрывает потребности пользователей, отслеживающих широкий портфель (S&P 500, секторы, геополитика).

#### Зависимости (8 файлов)

| # | Система | Файл | Строка | Что | Было | Стало |
|---|---------|------|--------|-----|------|-------|
| 1 | Backend | `src/services/digest.ts` | ~212 | `buildDigest` — LIMIT тегов | `? 10 : 1` | `? 25 : 1` |
| 2 | Backend | `src/services/digest.ts` | ~258 | `sendDigestToUserNow` — LIMIT | `? 10 : 1` | `? 25 : 1` |
| **3** | **Backend** | **`src/routes/user.ts`** | **~150** | **API — отклонение при лимите** 🔴 | **`? 10 : 3`** | **`? 25 : 3`** |
| 4 | Frontend | `src/pages/Home.tsx` | 144 | `tagLimit` — disabled state | `? 10 : 3` | `? 25 : 3` |
| 5 | Frontend | `src/pages/Pricing.tsx` | 37 | Текст на странице | `10 тегов` | `25 тегов` |
| 6 | Frontend | `src/pages/Profile.tsx` | 553 | Текст в карточке | `До 10` | `До 25` |
| 7 | Frontend | `src/components/PremiumPromptModal.tsx` | 84 | Текст в модалке | `До 10` | `До 25` |
| 8 | Frontend | `src/pages/Instructions.tsx` | 118 | Текст в инструкции | `(10 тегов)` | `(25 тегов)` |

#### 🔴 Критическая зависимость (почему user.ts — важнее всех)

**Если НЕ обновить `user.ts:150`:**
```
Frontend: кнопка "Добавить тег" активна (видит 25)
Backend API: отвечает 403 "Tag limit reached (10)"
Результат: пользователь не может добавить 11-й тег
```

**Проверка:** добавление 14-го тега на аккаунт admin → `{"tag": {...}}` ✅

#### Что НЕ меняется

| Файл | Почему |
|------|--------|
| `src/routes/payment.ts` | Цена остаётся 490 ₽ |
