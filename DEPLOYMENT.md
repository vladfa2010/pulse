...
**Принципы работы почты:**
- Единственный провайдер — Resend API (`EMAIL_PROVIDER=resend`); Yandex SMTP удалён
  как мёртвый код (опечатка `createTransporter` — отправка падала всегда, ошибка
  глоталась). `EMAIL_PROVIDER=none` — письма молча пропускаются (страховка).
- `EMAIL_FROM` обязан быть на верифицированном в Resend домене:
  `noreply@pulse.inside-trade.ru`. Дефолт в коде — тот же адрес (инцидент:
  дефолт `pulse.app` отклонялся Resend 403, сбой был невидим).
- При старте бэкенд пишет boot-лог `[Email] Config: provider=..., from=...,
  resend_key=set|MISSING` и WARNING error-уровня, если провайдер не задан или
  ключ отсутствует. Потеря env видна в логах сразу, не по жалобам пользователей.
- Сбой доставки письма НЕ пробрасывается пользователю: `sendEmail()` → false,
  для кода восстановления есть TG-fallback. Анти-перебор (одинаковый ответ для
  существующего/несуществующего email) не ломается никогда.

**Системные алерты** — `notifyAdminsSystemAlert(text)` в `src/services/adminAlerts.ts`:
рассылает ВСЕМ активным админам (`admin_tg_settings.is_active`) независимо от
подписок на event_types (в отличие от `notifyAdmins` — тот фильтрует; операционный
инцидент не должен зависеть от галочки в настройках). Точка вызова: провал
доставки кода восстановления пароля (email + TG-fallback). Вызов всегда
fire-and-forget (`.catch(() => {})`), ответ пользователю не задерживает.
...
- Импорт исторических каскадов перезапущен (`importCascadeSnapshot.ts`,
  2026-09-12): 1568 кластеров / 4225 связей восстановлены (217 id вне news —
...
- **Документация:** полное описание пайплайна, таблиц, API, флагов и
  инцидентов — `docs/topics.md` в этом репозитории.
- **Фронт:** вкладка «Темы» (позиция 3: Каскады → Сюжеты → Темы) только в
  ...
  ⚠️ Флаги затрагивают ТОЛЬКО «Темы»: вкладки «Каскады»/«Сюжеты» и их API
  (`/api/clusters*`, `/api/stories`) от env не зависят — работают всегда и
  везде. Второй флаг страницы, `VITE_CASCADE_EXPAND` (ТЗ-101), по умолчанию
  включён (выключается явным `'false'`).
...
### VPS: сайт 404 после деплоя фронта (Caddy)
**Причина:** Bind-mount Caddy смотрит на **inode каталога**, а не на путь. Если при деплое сделать `mv dist dist.old && mkdir dist && tar -xzf` — контейнер продолжает смотреть на старый (удалённый) inode, `/srv/dist` пуст → Caddy отдаёт 404 на всё. Инцидент 2026-09-15.
**Решение:** после распаковки в пересозданный каталог обязателен `docker restart pulse-caddy` (mount пересоздаётся по пути). Либо распаковывать поверх существующего каталога без `mv/mkdir`. Дополнительно: macOS-тар добавляет `._*` AppleDouble-файлы — паковать `COPYFILE_DISABLE=1 tar -czf ...`, на сервере чистить `find dist -name '._*' -delete`. Проверка после деплоя: `curl -w '%{http_code}' https://pulse.inside-trade.ru/` (ожидается 200).
