...
**Защита от дублей:**
- Перед созданием `createUserTag()` ищет тег в три этапа:
  1. точное совпадение по `tag_id`;
  2. точное совпадение по `LOWER(tag_name)`;
  3. совпадение по транслит-вариантам (`getTranslitVariants`): `tag_id IN (...)` или `LOWER(tag_name) IN (...)`.
- Если тег найден на любом этапе — пользователь подписывается на существующий `tag_id`, новая запись не создаётся.
- Это предотвращает дубли вида `sberbank` / `сбербанк`, `gazprom` / `газпром`, `yandex` / `яндекс`.
- Фонетические варианты (`apple` / `эппл`) не покрываются — известное ограничение.
...
**Примечание:** подписка всегда выполняется на канонический `tag_id`, найденный по `tag_id`, `LOWER(tag_name)` или транслит-варианту названия.
