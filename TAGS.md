...
> - `country` (страна регистрации / основного листинга; legacy-поле, синхронизируется с `geo_countries[0]`)
> - `geo_countries` (массив стран присутствия/регистрации, HQ-страна первой)
> - `geo_regions` (массив регионов/штатов/областей)
> - `geo_cities` (массив городов, HQ-город первым)
...
>
> **Note (v8.7+):** гео-поля (`country`, `geo_countries`, `geo_regions`, `geo_cities`) **НЕ**
> используются в keyword matching. Они хранятся в enriched-данных тега, отображаются в
> админской карточке и участвуют в поиске тегов (`/api/tags/search`), но не попадают в
> keywords — иначе новости с упоминанием "Москва" или "Россия" притягивались бы ко
> всем тегам с такой гео. Гео-матчинг новостей планируется отдельным NER-шагом в будущем.
...
- Гео-поля (`geo_countries`, `geo_regions`, `geo_cities`) хранятся в `enriched_data`. При сохранении `geo_countries` поле `country` автоматически синхронизируется с `geo_countries[0]`. Старые теги, где заполнен только `country`, отображаются как `geo_countries = [country]`. Гео-поля не включаются в keywords.
