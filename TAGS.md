...
Новость (title + summary)
  │
  ├──> Layer 1: Keyword Matching (local, fast)
  │     ├── Стандартные теги (18 шт) — hardcoded
  │     └── Пользовательские теги — из БД
  │
  ├──> Layer 2: LLM Smart Matching (Kimi API, async)
  │     └── Только если Layer 1 вернул []
  │
  └──> Layer 3: Related Tags (local, fast)
        └── nvda → tech + ai, etc.
```
...
## 3. Алгоритм матчинга (3 слоя)

### Layer 1: Keyword Matching
```typescript
function matchTagsByKeywords(text: string): string[]
```
- **Вход:** title + summary (конкатенация)
- **Логика:** lowerCase(text) → includes каждое keyword
- **Покрытие:** ~60-70% новостей
- **Скорость:** мгновенно (local)
...
```
- **Условие:** Layer 1 вернул [] И useLLM !== false И KIMI_API_KEY установлен
- **API:** Kimi (api.moonshot.ai), модель из KIMI_MODEL
- **Prompt:** список доступных тегов + title + summary → JSON array
...
```
- **Условие:** после Layer 1 или 2
- **Логика:** RELATED_TAGS[matchedTagId] → добавить связанные
- **Скорость:** мгновенно
