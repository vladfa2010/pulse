...
async function start() {
  ...
  const migrations = [
    ...
    { sql: `CREATE INDEX IF NOT EXISTS idx_portfolios_user_frozen ON portfolios(user_id, is_frozen)`, name: 'idx_portfolios_user_frozen' },
    { sql: `CREATE INDEX IF NOT EXISTS idx_news_tag_links_tag_id ON news_tag_links(tag_id)`, name: 'idx_news_tag_links_tag_id' },
    { sql: `CREATE INDEX IF NOT EXISTS idx_news_published_at ON news(published_at)`, name: 'idx_news_published_at' },
    ...
    { sql: `CREATE TABLE IF NOT EXISTS user_promo_uses (id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE, promo_code_id UUID NOT NULL REFERENCES promo_codes(id) ON DELETE CASCADE, plan_id VARCHAR(20) NOT NULL, billing_cycle VARCHAR(10) NOT NULL, discount_applied INTEGER NOT NULL DEFAULT 0, trial_days_used INTEGER DEFAULT NULL, expected_renewal_price DECIMAL(10,2) DEFAULT NULL, payment_id UUID REFERENCES payments(id), created_at TIMESTAMP DEFAULT ${_SQL_NOW})`, name: 'user_promo_uses' },
    { sql: `ALTER TABLE user_promo_uses DROP CONSTRAINT IF EXISTS user_promo_uses_user_id_promo_code_id_key`, name: 'drop_user_promo_uses_unique' },
    ...
    { sql: `CREATE TABLE IF NOT EXISTS features_registry (id VARCHAR(50) PRIMARY KEY, label VARCHAR(100) NOT NULL, description VARCHAR(255) DEFAULT NULL, is_active BOOLEAN NOT NULL DEFAULT TRUE, integration_status VARCHAR(20) NOT NULL DEFAULT 'pending', created_at TIMESTAMP DEFAULT ${_SQL_NOW}, updated_at TIMESTAMP DEFAULT ${_SQL_NOW})`, name: 'features_registry' },
    ...
    { sql: `ALTER TABLE features_registry ADD COLUMN IF NOT EXISTS integration_status VARCHAR(20) NOT NULL DEFAULT 'pending'`, name: 'features_registry_integration_status' },
    { sql: `UPDATE features_registry SET integration_status = 'integrated' WHERE is_active = TRUE AND integration_status = 'pending'`, name: 'features_registry_backfill_integration_status' },
    { sql: `INSERT INTO features_registry (id, label, description, integration_status) VALUES ('telegram', 'Telegram-дайджест', 'Дайджест новостей в Telegram', 'integrated'), ('push', 'Push-уведомления', 'Push-уведомления в браузере/приложении', 'integrated'), ('ai_summary', 'AI-саммари по портфелю', 'AI-анализ портфеля каждый час', 'pending'), ('alerts', 'Sentiment-алерты', 'Уведомления при резком изменении сентимента', 'pending'), ('priority', 'Приоритетная доставка', 'Приоритет обработки новостей', 'pending'), ('early_delivery', 'Ранняя доставка', 'Доступ к новостям на 5 минут раньше', 'pending'), ('custom_thresholds', 'Кастомные пороги', 'Настройка порогов для алертов', 'pending'), ('club_access', 'Club доступ', 'Доступ к закрытому Telegram-чату', 'pending'), ('api_access', 'API доступ', 'Доступ к REST API с токеном', 'pending') ON CONFLICT (id) DO NOTHING`, name: 'features_registry_seed' },
  ...
  try {
    await query(`
      INSERT INTO subscription_plans
        (id, name, price_monthly, price_yearly, price, billing_frequency, yearly_discount, tag_limit, features, display_order, is_active, is_popular, coming_soon_label, plan_level)
      VALUES
        ('free',    'Free',    0,    0,    0,     'monthly', 0,  3,
         '{"telegram":false,"push":false,"ai_summary":false,"alerts":false,"priority":"normal"}',
         1, TRUE, FALSE, NULL, 0),
        ('base',    'Base',    100,  960,  100,   'monthly', 20, 10,
         '{"telegram":true,"push":true,"ai_summary":false,"alerts":false,"priority":"normal"}',
         2, TRUE, FALSE, NULL, 1),
        ('premium', 'Premium', 990,  9504, 990,   'monthly', 20, 25,
         '{"telegram":true,"push":true,"ai_summary":true,"alerts":true,"priority":"high"}',
         3, TRUE, TRUE, NULL, 2),
        ('club',    'Club',    2500, 24000, 2500,  'monthly', 20, -1,
        ...
        ('pro',     'Pro',     2500, 24000, 2500,  'monthly', 20, -1,
