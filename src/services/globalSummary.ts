...
// ═══════════════════════════════════════════════════════════════════════════
// Public read-only access (гостевая главная)
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Публичная отдача кэша для гостей: только свежая запись, НИКОГДА не триггерит
 * генерацию (LLM-расход недопустим от анонимных хитов). Cron прогревает кэш
 * сам: warm-up через 3 минуты после boot + каждые 6 часов.
 * Нет кэша/протух — null → роут отвечает 404, фронт скрывает блок.
 */
export function getCachedGlobalSummary(): GlobalSummaryCache | null {
